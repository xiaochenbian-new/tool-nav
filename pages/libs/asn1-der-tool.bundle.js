(() => {
  // ../Ctool/packages/ctool-core/src/tools/asn1/lib/int10.js
  var max = 1e13;
  var Int10 = class {
    /**
    ...
    toString(base) {
      ...
      let b = this.buf, s = b[b.length - 1].toString();
  ...
  var int10_default = Int10;

  // ../Ctool/packages/ctool-core/src/tools/asn1/lib/oids.js
  var oids = {
  ...
  var oids_default = oids;

  // ../Ctool/packages/ctool-core/src/tools/asn1/lib/asn1.js
  var ellipsis = "\u2026";
  var reTimeS = /^(\d\d)(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])([01]\d|2[0-3])(?:([0-5]\d)(?:([0-5]\d)(?:[.,](\d{1,3}))?)?)?(Z|(-(?:0\d|1[0-2])|[+](?:0\d|1[0-4]))([0-5]\d)?)?$/;
  var reTimeL = /^(\d\d\d\d)(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])([01]\d|2[0-3])(?:([0-5]\d)(?:([0-5]\d)(?:[.,](\d{1,3}))?)?)?(Z|(-(?:0\d|1[0-2])|[+](?:0\d|1[0-4]))([0-5]\d)?)?$/;
  var hexDigits = "0123456789ABCDEF";
  var b64Safe = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_";
  var tableT61 = [
    ["", ""],
    ["AEIOUaeiou", "\xC0\xC8\xCC\xD2\xD9\xE0\xE8\xEC\xF2\xF9"],
    // Grave
    ["ACEILNORSUYZacegilnorsuyz", "\xC1\u0106\xC9\xCD\u0139\u0143\xD3\u0154\u015A\xDA\xDD\u0179\xE1\u0107\xE9\u0123\xED\u013A\u0144\xF3\u0155\u015B\xFA\xFD\u017A"],
    // Acute
    ["ACEGHIJOSUWYaceghijosuwy", "\xC2\u0108\xCA\u011C\u0124\xCE\u0134\xD4\u015C\xDB\u0174\u0176\xE2\u0109\xEA\u011D\u0125\xEE\u0135\xF4\u015D\xFB\u0175\u0177"],
    // Circumflex
    ["AINOUainou", "\xC3\u0128\xD1\xD5\u0168\xE3\u0129\xF1\xF5\u0169"],
    // Tilde
    ...
    ["CEGIZcegz", "\u010A\u0116\u0120\u0130\u017B\u010B\u0117\u0121\u017C"],
    // Dot
    ["AEIOUYaeiouy", "\xC4\xCB\xCF\xD6\xDC\u0178\xE4\xEB\xEF\xF6\xFC\xFF"],
    // Umlaut or diæresis
    ["", ""],
    ["AUau", "\xC5\u016E\xE5\u016F"],
    // Ring
    ["CGKLNRSTcklnrst", "\xC7\u0122\u0136\u013B\u0145\u0156\u015E\u0162\xE7\u0137\u013C\u0146\u0157\u015F\u0163"],
    // Cedilla
    ["", ""],
    ["OUou", "\u0150\u0170\u0151\u0171"],
    // Double Acute
  ...
  var Stream = class _Stream {
    ...
    hexByte(b) {
      return hexDigits.charAt(b >> 4 & 15) + hexDigits.charAt(b & 15);
    }
    ...
    b64Dump(start, end) {
      let extra = (end - start) % 3, s = "", i, c;
      for (i = start; i + 2 < end; i += 3) {
        c = this.get(i) << 16 | this.get(i + 1) << 8 | this.get(i + 2);
        s += b64Safe.charAt(c >> 18 & 63);
        s += b64Safe.charAt(c >> 12 & 63);
        s += b64Safe.charAt(c >> 6 & 63);
        s += b64Safe.charAt(c & 63);
      }
      if (extra > 0) {
        c = this.get(i) << 16;
        ...
        s += b64Safe.charAt(c >> 18 & 63);
        s += b64Safe.charAt(c >> 12 & 63);
        if (extra == 2) s += b64Safe.charAt(c >> 6 & 63);
      }
      return s;
    }
    isASCII(start, end) {
    ...
    parseStringT61(start, end, maxLength) {
      ...
      for (let i = start; i < end; ++i) {
        c = this.get(i);
        if (c >= 164 && c <= 191)
          s += "$\xA5#\xA7\xA4\0\0\xAB\0\0\0\0\xB0\xB1\xB2\xB3\xD7\xB5\xB6\xB7\xF7\0\0\xBB\xBC\xBD\xBE\xBF".charAt(c - 164);
        else if (c >= 224 && c <= 255)
          s += "\u2126\xC6\xD0\xAA\u0126\0\u0132\u013F\u0141\xD8\u0152\xBA\xDE\u0166\u014A\u0149\u0138\xE6\u0111\xF0\u0127\u0131\u0133\u0140\u0142\xF8\u0153\xDF\xFE\u0167\u014B\0".charAt(c - 224);
        else if (c >= 192 && c <= 207)
    ...
    parseStringUTF(start, end, maxLength) {
      ...
      for (let i = start; i < end; ) {
        ...
        if (c < 128)
          ...
          s += String.fromCharCode((c & 31) << 6 | ex(this.get(i++)));
        else if (c < 240)
          s += String.fromCharCode((c & 15) << 12 | ex(this.get(i++)) << 6 | ex(this.get(i++)));
        else if (c < 248)
          s += surrogate((c & 7) << 18 | ex(this.get(i++)) << 12 | ex(this.get(i++)) << 6 | ex(this.get(i++)));
        else
    ...
    parseTime(start, end, shortYear) {
      let s = this.parseStringISO(start, end).str, m = (shortYear ? reTimeS : reTimeL).exec(s);
      if (!m)
    ...
    parseInteger(start, end) {
      let v = this.get(start), neg = v > 127, pad = neg ? 255 : 0, len, s = "";
    ...
    parseBitString(start, end, maxLength) {
      ...
      let lenBit = (end - start - 1 << 3) - unusedBits, s = "";
      for (let i = start + 1; i < end; ++i) {
        let b = this.get(i), skip = i == end - 1 ? unusedBits : 0;
    ...
    parseOID(start, end, maxLength, isRelative) {
      let s = "", n = new int10_default(), bits = 0;
      for (let i = start; i < end; ++i) {
        let v = this.get(i);
        n.mulAdd(128, v & 127);
        bits += 7;
        if (!(v & 128)) {
          if (s === "") {
            n = n.simplify();
            if (isRelative) {
              s = n instanceof int10_default ? n.toString() : "" + n;
            } else if (n instanceof int10_default) {
              n.sub(80);
              s = "2." + n.toString();
            } else {
      ...
      if (bits > 0)
        s += ".incomplete";
      if (typeof oids_default === "object" && !isRelative) {
        let oid = oids_default[s];
        if (oid) {
          if (oid.d) s += "\n" + oid.d;
          if (oid.c) s += "\n" + oid.c;
          if (oid.w) s += "\n(warning!)";
  ...
  function recurse(el, parser, maxLength) {
    ...
    let d = { size: 0, str: "" };
    el.sub.forEach(function(el2) {
      let d1 = recurse(el2, parser, maxLength - d.str.length);
      d.size += d1.size;
      d.str += d1.str;
    });
    return d;
  ...
  var ASN1 = class _ASN1 {
    ...
    toPrettyString(indent) {
      ...
      if (this.sub !== null) {
        indent += "  ";
        for (let i = 0, max2 = this.sub.length; i < max2; ++i)
    ...
    static decodeLength(stream) {
      let buf = stream.get(), len = buf & 127;
      if (len == buf)
        return len;
      if (len === 0)
        return null;
    ...
    static decode(stream, offset, type = _ASN1) {
      if (!(type == _ASN1 || type.prototype instanceof _ASN1))
        throw "Must pass a class that extends ASN1";
      if (!(stream instanceof Stream))
        stream = new Stream(stream, offset || 0);
      let streamStart = new Stream(stream), tag = new ASN1Tag(stream), tagLen = stream.pos - streamStart.pos, len = _ASN1.decodeLength(stream), start = stream.pos, header = start - streamStart.pos, sub = null, getSub = function() {
        sub = [];
        if (len !== null) {
          let end = start + len;
  ...
  var asn1_default = ASN1;

  // ../Ctool/packages/ctool-core/src/tools/asn1/lib/asn1toxml.js
  function contentSimple(asn1, maxLength) {
    if (asn1.tag === void 0)
      return null;
    if (maxLength === void 0)
      maxLength = Infinity;
    let content = asn1.posContent(), len = Math.abs(asn1.length);
    ...
    switch (asn1.tag.tagNumber) {
      ...
      case 2:
        let d = recurse2(asn1, "parseOctetString", maxLength);
        return "0x" + d.str;
      case 3: {
        let d2 = recurse2(asn1, "parseOctetString", maxLength);
        return "0x" + d2.str;
      }
      case 4: {
        let d2 = recurse2(asn1, "parseOctetString", maxLength);
      ...
      case 17:
        if (asn1.sub !== null)
          ...
          return "(no elem)";
      case 12:
        return recurse2(asn1, "parseStringUTF", maxLength).str;
      case 20:
        return recurse2(asn1, "parseStringT61", maxLength).str;
      case 18:
      // NumericString
      case 19:
      // PrintableString
      ...
      // VisibleString
      case 27:
        return recurse2(asn1, "parseStringISO", maxLength).str;
      case 30:
        return recurse2(asn1, "parseStringBMP", maxLength).str;
      case 23:
      // UTCTime
      case 24:
        return asn1.stream.parseTime(content, content + len, asn1.tag.tagNumber == 23);
    }
    return null;
  }
  function recurse2(el, parser, maxLength) {
    let avoidRecurse = true;
    if (el.tag.tagConstructed && el.sub) {
      avoidRecurse = false;
      el.sub.forEach(function(e1) {
    ...
    let d = { size: 0, str: "" };
    el.sub.forEach(function(el2) {
      let d1 = recurse2(el2, parser, maxLength - d.str.length);
      d.size += d1.size;
      d.str += d1.str;
    });
    return d;
  }
  var ASN1TOXML = class _ASN1TOXML {
    static decode2Xml(asn1) {
      ...
      let content = contentSimple(asn1, void 0);
      if (content) s += content.replace(/\n/g, "|");
      if (asn1.sub !== null) {
        for (let i = 0, max2 = asn1.sub.length; i < max2; ++i) {
          s += "\n" + _ASN1TOXML.decode2Xml(asn1.sub[i]);
        }
        if (typeName.startsWith("[")) {
          s += "\n</NODE>";
        } else if (typeName !== void 0) {
          s += "\n</" + typeName + ">";
        }
      } else {
        if (typeName.startsWith("[")) {
  ...
  var asn1toxml_default = ASN1TOXML;

  // scripts/asn1-bundle-entry.mjs
  window.ASN1 = asn1_default;
  window.ASN1TOXML = asn1toxml_default;
})();
